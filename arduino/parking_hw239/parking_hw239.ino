...
// This build is for the taller 0.96" module — the one silkscreened HW-239.
// Same SSD1306 controller, same four wires, same I2C address as the 128x32
// board; it just has twice the rows, so only the height and the screen layout
// change. If your module says SH1106 rather than SSD1306, this will show a
// shifted or blank image and needs a different library instead.
...
// The 128x64 screen buffer is 1024 bytes on the heap — twice the 128x32 one —
// and the compiler's "global variables" figure does not include it. That
// leaves the stack noticeably tighter here, so the board reports what is
// actually free. Below about 200 bytes, expect random resets.
int freeRam() {
...
void printConfig() {
  ...
  Serial.print(F(" mode="));      Serial.print(manualMode ? F("MANUAL") : F("AUTO"));
