// tacho_uno.ino - analogue rev counter and speedometer, on an Arduino Uno.
//
...
//
//   rev counter servo signal -> pin 9
//   speedometer servo signal -> pin 10
//
...
// GOING THE WRONG WAY? Swap the two numbers so MIN is the larger. Which way a
// servo turns for a rising angle depends on how its horn is splined and which
// way round it is mounted, so half of them read backwards. Both gauges here
// are set reversed, matching the servos on the built dashboard.
#define TACHO_ENABLED   1
#define TACHO_PIN       9
#define TACHO_MIN_DEG 170     // reversed
#define TACHO_MAX_DEG  10

#define SPEEDO_ENABLED   1
#define SPEEDO_PIN      10
#define SPEEDO_MIN_DEG 170    // reversed
#define SPEEDO_MAX_DEG  10
...
bool gaugeCommand(Gauge &g, const char *name) {
  char *s = strtok(NULL, " ");
  if (!s) {
...
void handleCommand(char *line) {
  ...
  if (!strcasecmp(cmd, "PING")) { Serial.println(F("OK PONG tacho_uno " VERSION)); return; }
...
void setup() {
  ...
  Serial.print(F("# tacho_uno " VERSION " ready, free="));
...
void loop() {
  ...
  if (now - lastServoMs >= SERVO_MS) {
    ...
    // times a second, which is far smoother than a needle needs, but the two
    // servos never draw their step current in the same instant. On a board
    // with no reservoir capacitor that halves the worst-case sag on the 5V
    // rail, which is what causes brown-out resets.
    servoTurn ^= 1;
