#!/usr/bin/env python3
"""Live dashboard for the temp_serial Arduino firmware (DHT11).

Owns the serial port, streams telemetry to the browser over SSE, relays
commands back, and can flash the board from the browser by handing the port to
avrdude and taking it back afterwards.

    python dashboard.py                 # COM7, http://127.0.0.1:8790
    python dashboard.py --port COM3 --http-port 9003 --no-open --lan
"""
...
HISTORY = 720                 # 2.5s apart -> about half an hour

# A DHT11 only answers every couple of seconds, so "recently heard from" has to
# be a much wider window here than on the 10Hz projects.
STALE_AFTER = 6.0

SKETCH_INFO = {
    "temp_serial": {
        "title": "Thermometer + dashboard",
        "blurb": "Reads the temperature and humidity and shows them on this website.",
        "needed": True,
    },
}
...
class Board:
    ...
    def suspend(self, timeout: float = 8.0) -> bool:
        """Release the port for avrdude. Flag-based on purpose: closing a
        pyserial handle from another thread while readline() blocks tears down
        the Windows overlapped-IO state and kills the reader."""
        self._suspend = True
        ...
        def do_GET(self):
            ...
            if self.path == "/api/sketches":
                self._send(200, json.dumps({
                    "sketches": flasher.sketches(), "busy": flasher.busy,
                    "port": board.port, "health": board.health(),
...
def main() -> None:
    ...
    print(f"thermometer: {url}   (serial {args.port} @ {args.baud})")
    if args.lan:
