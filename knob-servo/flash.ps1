<#
.SYNOPSIS
  Compile and upload an Arduino sketch headlessly using the Arduino IDE 1.8.x
  toolchain. Defaults to the Mega 2560 this project runs on.

.EXAMPLE
  .\flash.ps1 knob -VerifyOnly     # compile only, don't touch the board
...
param(
  ...
  # A Mega's bootloader speaks STK500v2, so avrdude needs 'wiring' here.
  # An Uno would be 'arduino'.
  [string]$Programmer = 'wiring',
  [int]   $Baud = 115200,
  [switch]$VerifyOnly
)
...
& "$ide\hardware\tools\avr\bin\avrdude.exe" `
  ...
  -p $Mcu -c $Programmer -P $Port -b $Baud -D `
  -U "flash:w:${hex}:i"
