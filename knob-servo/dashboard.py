#!/usr/bin/env python3
"""Live dashboard for the knob (potentiometer -> servo) firmware.

    python dashboard.py                 # COM10, http://127.0.0.1:8792
    python dashboard.py --port COM3 --http-port 9004 --no-open --lan
"""
...
FLASH_LOG_BOARD = "Arduino Mega 2560"
...
SKETCH_INFO = {
    "knob": {
        "title": "Knob controls the servo",
        "blurb": "Turn the knob, the servo follows. Also lets this page drive it.",
        "needed": True,
    },
}
...
class Board:
    ...
    def health(self) -> str:
        ...
        if not self.connected:
            return "offline"
        return "ok" if (time.time() - self.last_telem) < STALE_AFTER else "silent"

    def snapshot(self) -> dict:
        return {"type": "snapshot", "connected": self.connected, "health": self.health(),
                "port": self.port, "config": self.config,
                "history": list(self.history), "log": list(self.log)}
    ...
    def _handle(self, line: str) -> None:
        ...
        if m:
            point = {"t": int(m.group(1)), "raw": int(m.group(2)),
                     "a": int(m.group(3)), "mode": m.group(4)}
    ...
    def suspend(self, timeout: float = 8.0) -> bool:
        """Flag-based: closing a pyserial handle from another thread while
        readline() blocks tears down the Windows overlapped-IO state."""
        self._suspend = True
...
class Flasher:
    ...
    def sketches(self) -> list[dict]:
        ...
        for d in sorted(HERE.iterdir()):
            ...
            out.append({"name": d.name, "title": info.get("title", d.name),
                        "blurb": info.get("blurb", ""), "needed": info.get("needed", False)})
        return out
    ...
    def run(self, sketch: str) -> None:
        if sketch not in {s["name"] for s in self.sketches()}:
        ...
        try:
            ...
            if not self.board.suspend():
                self._emit("Couldn't free the USB port. Unplug the board and plug it back in.", "error")
                return
            time.sleep(0.4)
            self._emit(f"Sending '{sketch}' to your Arduino. This takes about ten seconds.", "step")
            proc = subprocess.Popen(
                ["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass",
                 "-File", str(HERE / "flash.ps1"), "-Sketch", sketch, "-Port", self.port],
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if self.path == "/api/sketches":
                self._send(200, json.dumps({"sketches": flasher.sketches(), "busy": flasher.busy,
                                            "port": board.port, "health": board.health()}).encode(),
                           "application/json")
                return
        ...
        def do_POST(self):
            ...
            if self.path == "/api/flash":
                try:
                    sketch = str(json.loads(body or b"{}").get("sketch", "")).strip()
            ...
            if self.path == "/cmd":
                try:
                    cmd = str(json.loads(body or b"{}").get("cmd", "")).strip()
                except (ValueError, UnicodeDecodeError):
                    self._send(400, b'{"ok":false}', "application/json")
                    return
                if not cmd or "\n" in cmd:
                    ...
                    return
                ok = board.send(cmd)
                self._send(200 if ok else 503, json.dumps({"ok": ok}).encode(), "application/json")
                return
            self._send(404, b"not found")
...
def main() -> None:
    ...
    ap.add_argument("--http-port", type=int, default=8792)
    ...
    print(f"knob dashboard: {url}   (serial {args.port} @ {args.baud})")
    if args.lan:
